<style>
    <?php include('files/css/our_values.css'); ?>


</style>


<div class="our_values col-12" style="margin-top: 45px; padding-right: 0px;">

    <!-- Accordion Element -->
    <div class="accordion">

        <div class="accordion__title d-flex align-items-center justify-content-between">
            <div class="accordion__title__text">Our Processing</div>
            <div class="accordion__title__icon d-flex"><i class="material-icons">add</i></div>
        </div>

        <div class="accordion__content animated fadeIn">
            <div class="flex_container d-flex justify-content-between flex-wrap">
                <div class="col-12 accordion__content__rhs">
                    <div class="accordion__content__rhs__title">
                        <h5>Lorem, ipsum dolor.</h5>
                        <p>Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Duis aute irure dolor.</p>
                    </div>
                    <div class="accordion__content__rhs__description"></div>
                </div>
            </div> 
        </div>

    </div>
    <!-- Accordion Element -->

    <!-- Accordion Element -->
    <div class="accordion">
        <div class="accordion__title d-flex align-items-center justify-content-between ">
            <div class="accordion__title__text">Advanced Research</div>
            <div class="accordion__title__icon d-flex"><i class="material-icons">add</i></div>
        </div>

        <div class="accordion__content animated fadeIn">
            <div class="flex_container d-flex justify-content-between flex-wrap">
                <div class="col-12 accordion__content__rhs">
                    <div class="accordion__content__rhs__title">
                        <h5>Lorem, ipsum dolor.</h5>
                        <p>Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Duis aute irure dolor</p>
                    </div>
                    <div class="accordion__content__rhs__description"></div>
                </div>
            </div> 
        </div>

    </div>
    <!-- Accordion Element -->

    <!-- Accordion Element -->
    <div class="accordion">

        <div class="accordion__title d-flex align-items-center justify-content-between ">
            <div class="accordion__title__text">Next-Gen Machinery</div>
            <div class="accordion__title__icon d-flex"><i class="material-icons">add</i></div>
        </div>

        <div class="accordion__content animated fadeIn">
            <div class="flex_container d-flex justify-content-between flex-wrap">

                <div class="col-12 accordion__content__rhs">
                    <div class="accordion__content__rhs__title">
                        <h5>Lorem, ipsum dolor.</h5>
                        <p>Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Duis aute irure dolor</p>
                    </div>
                    <div class="accordion__content__rhs__description"></div>
                </div>
            </div> 
        </div>

    </div>
    <!-- Accordion Element -->

</div>


<script>
    <?php include('files/js/our_values.js'); ?>
</script>